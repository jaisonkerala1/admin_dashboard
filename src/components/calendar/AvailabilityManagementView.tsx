    ...
    <div className="space-y-4">
      <div className="flex flex-col lg:flex-row gap-3 lg:items-end lg:justify-between">
        <div className="max-w-xl w-full">
          <label className="block text-sm font-medium text-gray-700 mb-1">Select astrologer</label>
          <select
        ...
        <button
          ...
          className="inline-flex items-center justify-center gap-2 px-4 py-2 rounded-lg bg-primary-600 text-white font-medium hover:bg-primary-700 disabled:opacity-60 disabled:cursor-not-allowed"
        >
      ...
      {!selectedAstrologerId ? (
        <div className="p-10 text-center text-gray-500">Select an astrologer to manage availability.</div>
      ) : (
        <div className="space-y-3">
          {selectedAvailability.length === 0 ? (
            <div className="p-10 text-center text-gray-500">No availability rules found.</div>
          ) : (
            selectedAvailability
              .slice()
              .sort((a, b) => a.dayOfWeek - b.dayOfWeek)
              .map((a) => (
                <div key={a._id} className="p-4 rounded-xl border border-gray-200 bg-white flex items-start justify-between gap-3">
                  <div>
                    <div className="font-semibold text-gray-900">
                      {dayNames[a.dayOfWeek] || `Day ${a.dayOfWeek}`} • {a.startTime}-{a.endTime}{' '}
                      {!a.isActive && <span className="text-xs text-gray-500">(inactive)</span>}
                    </div>
                    {a.breaks?.length > 0 && (
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      type="button"
                      onClick={() => openEdit(a)}
                      className="inline-flex items-center gap-2 px-3 py-2 rounded-lg bg-gray-50 hover:bg-gray-100 border border-gray-200 text-sm font-medium"
                    >
          ...
          <div className="space-y-4">
            ...
            <div className="border-t border-gray-100 pt-4">
              <div className="flex items-center justify-between">
                <div className="text-sm font-semibold text-gray-900">Breaks</div>
                <button
                  type="button"
                  disabled={isSaving}
                  onClick={() =>
              ...
              {draft.breaks.length === 0 ? (
                <div className="text-sm text-gray-500 mt-2">No breaks.</div>
              ) : (
